 ...
 * - Creator mode (no restaurant): AdvancedDesignEditorConfig
 */
    ...
    <div className={styles.overlay}>
      <div className={styles.modal}>
        <CreativeEditor
          ...
          init={async (cesdk) => {
            const initOptions = { title, onBack, onSave };

            // Initialize appropriate editor configuration based on mode
            if (selectedRestaurant) {
              await initDesignEditor(cesdk, initOptions);
            } else {
              await initAdvancedDesignEditor(cesdk, initOptions);
            }

            // Load scene
            await cesdk.engine.scene.loadFromString(sceneToLoad);
